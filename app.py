...
if page == "Executive Overview":
    ...
    c1.metric("Total Revenue", f"TRY {kpis['total_revenue']:,.0f}")
    c2.metric("Total Orders", f"{kpis['total_orders']:,}")
    c3.metric("Critical Parts", kpis["critical_parts"])
    c4.metric("Average Risk", f"{kpis['avg_risk']:.1f}/100")
    c5.metric("Delayed Orders", delayed_orders)
    c6.metric("Rejected Orders", rejected_orders)
    ...
    with left:
        st.write("### Monthly Order and Revenue Trend")
        ...
        st.line_chart(monthly.set_index("OrderDate")[["Orders", "Revenue"]])
